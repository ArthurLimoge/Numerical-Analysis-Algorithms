import numpy as np
from Parallel_QR_Reduction import parallel_qr_reduction

def least_squares(_input, target):
    _input = np.array(_input)
    row_dim = _input.shape[0]
    col_dim = _input.shape[1]
    table = qr_reduction(_input)  # if the matrix has significantly more rows, use parallel QR reduction instead (line below)
#   table = parallel_qr_reduction(_input)
    _input, orthogonal, matrix = table[0], table[1], table[2]
    rhs = np.matmul(np.transpose(orthogonal), target)  # right-hand side Q^t * b
    sol_criterion = np.prod(rhs[col_dim:])  # there exists an exact solution iff Q^t * b only has non-zero components in the first 'col_dim' rows
    if sol_criterion == 0:
